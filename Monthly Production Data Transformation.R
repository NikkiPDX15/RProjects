...
clean<- data.frame(na.omit(projects000[5:8])) # this might just be a condition of the import
...
for (yy in 1:3){
  for (iii in 1:(nrow(DayPlay))){
    ...
    DayPlay[print22,mm+1] = Day22$usageamount
  }
  yy<- yy+1
...
for (y in 1:9){
  for (i in 1:(nrow(DayPlay))){
    Day <- data.frame(subset(clean,clean$date == YEARDATE)) # narrow by date
    print1<- as.array(which(DayPlay$Project %in% Day$facilityname)) # pick like terms
    DayPlay[print1,m+1] = Day$usageamount
  }
  y<- y+1
  YEARDATE <- YEARDATE +1
  m<- m+1
...
for (yy in 1:2){
  for (iii in 1:(nrow(DayPlay))){
    Day22 <- data.frame(subset(clean,clean$date == YEARDATE2)) # narrow by date
    print22<- as.array(which(DayPlay$Project %in% Day22$facilityname)) # pick like terms
    DayPlay[print22,yyy+1] = Day22$usageamount
  }
  yyy<- yyy+1
  YEARDATE2 <- YEARDATE2 +1
}

write.table(DayPlay, file = "Montly Production Values.csv",row.names=FALSE, na="",
            col.names=TRUE, sep=",")
